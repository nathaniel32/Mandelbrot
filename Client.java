...
public class Client extends UnicastRemoteObject implements ClientInterface {
    ...
    Client() throws RemoteException {
        ...
        ApfelModel m = new ApfelModel(v, this);
    ...
    public static void main(String[] args) {
        if (args.length == 1){
            try {
                ...
                String clientUrl = "rmi://" + currentIP + ":" + clientPort + "/ClientServer";

                Client client = new Client();
                LocateRegistry.createRegistry(clientPort);
                Naming.rebind(clientUrl, client);

                System.out.println("ClientServer ist gestartet...\nURL: " + clientUrl + "\n\n");
            } catch (Exception e) {
...
class ApfelModel {
    ...
    public ApfelModel(ApfelView v, Client client) {
        this.v = v;
        this.client = client;
    }
    ...
    class ApfelWorker implements Runnable {
        ...
        public void run() {
            try {
                int result_index = 0;
                int[][] result = client.bild_rechnen(v.workers_threads, worker_max_iter, v.max_betrag, worker_y_start, worker_y_stop, xpix, ypix, worker_xmin, worker_xmax, worker_ymin, worker_ymax);
